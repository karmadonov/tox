        ...
        .for_each(move |(_relay_pk, packet)| { // TODO: do we need relay_pk at all?
            let future = match packet {
                IncomingPacket::Data(sender_pk, packet) => match packet {
                    DataPayload::CookieRequest(packet) => Box::new(net_crypto_c.handle_tcp_cookie_request(&packet, sender_pk).map_err(Error::from)) as Box<Future<Item = _, Error = _> + Send>,
                ...
                IncomingPacket::Onion(packet) => match packet {
                    InnerOnionResponse::OnionAnnounceResponse(packet) => Box::new(onion_client_c.handle_announce_response(&packet, true).map_err(Error::from)) as Box<Future<Item = _, Error = _> + Send>,
